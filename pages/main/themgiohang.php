...
//         // kiem tra seesion gio hang ton ta
...
if (isset($_POST['themgiohang'])) {
    ...
    if ($row) {
        ...
        // Kiểm tra xem có session giỏ hàng không
        if (isset($_SESSION['cart'])) {
            $found = false;
            foreach ($_SESSION['cart'] as &$cart_item) {
            ...
            if ($found == false) {
                // Nếu không tìm thấy sản phẩm trong giỏ hàng, thêm sản phẩm mới
                $_SESSION['cart'][] = $new_product;
            }
        } else {
            // Nếu chưa có giỏ hàng, tạo giỏ hàng mới với sản phẩm
            $_SESSION['cart'][] = $new_product;
        }
    }
